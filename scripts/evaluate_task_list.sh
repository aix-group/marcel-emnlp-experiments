...
echo sbatch --array=0-$(($n_tasks - 1))%20 scripts/evaluate.sh --model neuralmagic/Meta-Llama-3.1-70B-Instruct-quantized.w8a8 --tensor_parallel_size 2 --metrics GeneratedAnswerLength,ReferenceAnswerLength,ContextLength,MeanReciprocalRank,PrecisionAtCutoff,RecallAtCutoff,ContextPrecision,ContextRecall,BLEU,ROUGE,BERTScore,AnswerSimilarity,AnswerFaithfulness,AnswerRelevance,NonAnswerCritic
